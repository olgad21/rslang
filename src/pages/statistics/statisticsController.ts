...
const updateDailyStatistics = () => {
  ...
  const dailyLearnedWords = document.querySelector('[data-id="daily-learned-words"]');
  const dailyRightWords = document.querySelector('[data-id="daily-right-words"]');
};
...
const updateGameStatistics = () => {
  ...
  Array.from(allGamesData).forEach((game) => {
    ...
    const gameLearnedWords = document.querySelector('[data-id="learned-words"]');
